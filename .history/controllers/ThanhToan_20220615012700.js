...
class thanhToanController {
  ...
  async addTable(req,res,next){
    try{
      ...
      var sql_query =
      "select count(SoBan) as Length from BanAn";
      var length=await request.query(sql_query, function (err, recordset) {
      if (err) res.send(err);
      console.log(recordset.recordset[0].Length);
      return(  Number(recordset.recordset[0].Length));
    });
    
      sql_query="insert into BanAn(IDBan,SoBan) values("+length+","+length+")";
