...
class thucDonController {
  ...
  async search(req, res, next) {
    try{var request = new sql.Request();
      var keyWord = Number(req.params.keyword);
      var sql_query"select IDMon,TenMon,GiaBan,TenPhanLoai,Image from MonAn,PhanLoai where MonAn.IDPhanLoai=PhanLoai.IDPhanLoai and MonAn.TenMon Like %" + keyWord +"%";
      await request.query(sql_query, function (err, recordset) {
        if (err) res.send(err);
        res.send({ result: recordset.recordset });
      });}
