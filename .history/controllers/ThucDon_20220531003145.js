...
class thucDonController {
  ...
  async searchWithIDCate(req, res, next) {
    try{
      ...
      await var sql_query = "select IDMon,TenMon,GiaBan,GiaTien,TenPhanLoai,Image from MonAn,PhanLoai where MonAn.IDPhanLoai=PhanLoai.IDPhanLoai and MonAn.IDPhanLoai="+IDCate;
      await request.query(sql_query, async function (err, recordset) {
        if (err) res.send(err);
        await res.send({ result: recordset.recordset });
      });}
