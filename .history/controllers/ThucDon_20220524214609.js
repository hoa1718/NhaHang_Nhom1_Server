...
class thucDonController{
    ...
    async detail(req,res,next){
      var request = new sql.Request();
      var id = (Numberreq.params.id;
      var sql_query="select IDMon,TenMon,GiaBan,TenPhanLoai,Image from MonAn,PhanLoai where MonAn.IDPhanLoai=PhanLoai.IDPhanLoai and MonAn.IDMon ="+ (Number)req.params.id;
        request.query(sql_query, function (err, recordset) {
            if (err)res.send(err) 
            res.send({result:recordset.recordset});
        });
